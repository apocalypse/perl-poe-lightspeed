...
our $VERSION = '1.' . sprintf( "%04d", (qw($Revision: 1115 $))[1] );
...
sub Link_Up {
	...
	# More sanity check
	if ( $kernel eq $remotekernel ) {
		die 'Using Lightspeed to connect to itself isn't very useful!';
	}

	# We do some sanity checks here
	if ( $_[HEAP]->{'GRAPH'}->has_edge( $kernel, $remotekernel ) ) {
		# Wow, we are double-connected to somebody...
		warn "Detected 2nd link to $remotekernel";
...
sub A_POST {
	...
	if ( FindOurself( $_[HEAP]->{'MYKERNEL'}, $msg->[ MSG_TO ] ) ) {
		...
		# Determine the type
		my $type;
		if ( $_[STATE] eq 'ACTION_' . ACTION_POST . '' ) {
			$type = 'post';
		} else {
			$type = 'callreply';
		}
